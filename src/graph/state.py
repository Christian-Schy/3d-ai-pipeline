...
class PipelineState(TypedDict):
    ...
    # --- Feature Assigner (Phase 1 — Häppchen) ---
    feature_assignments: dict
    """Per-feature structure assignments from FeatureAssignerAgent.
    Maps feature_id → {parent, operation, params}.
    Written by feature_assigner_node. Read by BlueprintAssembler.
    Example: {"plate_right": {"parent": "base", "operation": "add", "params": {"x": 20, "y": 80, "z": 40}}}"""

    # --- Position Assigners (Phase 1 — Häppchen, Split Architecture) ---
    position_assignments: dict
    """LEGACY: Combined position assignments. Kept for backward compatibility.
    New code should use feature_position_assignments + part_position_assignments."""

    feature_position_assignments: dict
    """Per-feature position assignments for subtract/modify features.
    Maps feature_id → {face, alignment, offset_x, offset_y, orientation_hint, face_hint, axis_hint}.
    Written by feature_position_assigner_node. Read by BlueprintAssembler.
    Example: {"hole_1": {"face": ">Z", "alignment": "centered", "offset_x": null, "offset_y": -5.0}}"""

    part_position_assignments: dict
    """Per-part position assignments for add-operation parts.
    Maps feature_id → {face, alignment, offset_x, offset_y, orientation_hint, face_hint,
                        distance_mm, gap_mm, relative_to}.
    Written by part_position_assigner_node. Read by BlueprintAssembler.
    Example: {"plate_right": {"face": ">Z", "alignment": "flush_right", "distance_mm": null}}"""
    ...
    # --- Agent Dispatcher ---
    active_agents: list
    """List of agent names that should run in this pipeline execution.
    Set by agent_dispatcher_node based on Feature Tagger output.
    Agents not in this list skip their LLM call and return empty dict."""

    agent_flags: list
    """Conditional behavior flags set by agent_dispatcher_node.
    Examples: 'inject_cylinder_rag', 'inject_shape_rag'.
    Read by downstream agents (e.g., Coder) for feature-specific RAG injection."""

    # --- Validation Feedback Routing ---
    validation_feedback_target: str
    """Which specialist agent should handle the validation feedback.
    Set by coordinate_validator or plan_validator when routing back.
    Values: 'feature_assigner', 'feature_position_assigner',
            'part_position_assigner', 'blueprint_assembler', or ''."""
